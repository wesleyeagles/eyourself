...
var JotForm = {
        ...
        $H(_data).each(function(pair) {
            ...
            JotForm.onTranslationsFetch(function() {
                $$('.form-checkbox%s, .form-radio%s'.replace(/\%s/gim, n)).each(function(input) {
                    ...
                    var allTranslations = [];
                    if (typeof FormTranslation !== 'undefined') {
                        allTranslations = Object.values(FormTranslation.dictionary).map(function(e) {
                    ...
                    if (allTranslations.length === 0) {
                        allTranslations.push(input.value);
                    }
    ...
    checkCalculation: function(calc) {
        ...
        try {
            if (!['text', 'email', 'textarea', 'calculation', 'combined', 'address', 'datetime', 'time', 'html', 'authnet', 'paypalpro', 'number', 'radio', 'checkbox', 'select', 'matrix', 'widget', 'signature', 'braintree', 'stripe', 'square', 'eway', 'bluepay', 'firstdata', 'chargify', 'echeck', 'payu', 'pagseguro', 'moneris', 'paypal', 'dwolla', 'bluesnap', 'paymentwall', 'payment', 'paypalexpress', 'payjunction', '2co', 'cardconnect', 'clickbank', 'onebip', 'worldpay', 'rating', 'hidden', 'file', 'other', 'mixed', 'sofort', 'payoneer', 'paysafe', 'gocardless', 'stripeACH', 'paypalSPB', 'cybersource', "paypalcomplete", 'inline', 'appointment', 'stripeCheckout', 'payfast'].include(JotForm.calculationType(result))) return;
        } catch (e) {
            console.log(e);
        }
        var combinedObject = {};
            ...
            switch (type) {
                ...
                case 'appointment':
                    ...
                    if (!selection.length) {
                        ...
                        val = selection[0].innerText.replace(/â€Ž/g, '');
                    }
                    break;
                default:
                    if ($('input_' + data) && typeof $('input_' + data).value !== 'undefined') {
